...
- **Nested-in-shadow parts are forwarded on demand under a `<child>-<part>` convention.** `::part()` does not pierce nested shadow roots: when component A renders another component B *inside its own shadow DOM* (not via a slot — slotted content stays in the consumer's light DOM and is reachable directly), B's parts are invisible to the consumer. To expose one, A forwards it with `exportparts` on the B element, renamed `<b>-<part>` (B's tag minus the `c-` prefix), e.g. `<c-divider exportparts="root:divider-root" />` → `c-side-navigation-title::part(divider-root)`. The rename is required: the bare inner name (e.g. `root`) would collide with A's own same-named part. This is done **on demand** — only when a nested region is a genuine customization target — rather than blanket-forwarding every nested component, which would leak implementation-detail parts (icons, spinners, inputs) into the public contract. Slotted children need no forwarding.
