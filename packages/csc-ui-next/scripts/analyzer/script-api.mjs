...
const interfaceMembers = (decl, sf, defaults = new Map()) =>
 ...
 */
export const analyzeScript = (content, fileName, className) => {
  ...
  if (definePropsType) {
    if (ts.isTypeReferenceNode(definePropsType)) {
      ...
      if (iface) props = interfaceMembers(iface, sf, defaults);
    } else if (ts.isTypeLiteralNode(definePropsType)) {
      props = definePropsType.members
  ...
  const events = eventMap
    ? interfaceMembers(eventMap, sf).map(({ description, name, type }) => ({
        description,
        detailType: type,
        name,
      }))
