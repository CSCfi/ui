...
| `renderOutsideShadow` | `render-outside-shadow` | Render input element outside shadow DOM for maximum compatibility | `boolean` | `false`            |
...
| `--c-text-field-label-color`              | Text field label color (works only with `label-on-top`) |
...
graph TD;
  ...
  c-input --> c-message
  style c-text-field-open fill:#f9f,stroke:#333,stroke-width:4px
```
