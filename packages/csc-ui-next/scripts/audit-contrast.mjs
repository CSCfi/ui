...
// --- pairs ----------------------------------------------------------------
const ROLES = ['primary', 'secondary', 'accent', 'success', 'info', 'warning', 'error', 'link'];
...
for (const bg of ['surface', 'surface-raised', 'surface-overlay', 'surface-muted']) {
...
add('border-strong', 'surface', UI, 'border-strong / surface (control outline)');
add('ring', 'surface', UI, 'ring / surface (focus)');
add('primary', 'surface', UI, 'primary / surface (icon/fill)');

// --- run ------------------------------------------------------------------
const modes = { light: resolve(light), dark: resolve(dark) };
let failures = 0;
let textFailures = 0;
