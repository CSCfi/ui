...
**Theme mode**:
The light-or-dark state that selects which palette tokens the **semantic tokens** resolve to. Pinned by the consumer on the document root, or on any element to cover just that subtree — see **mode scope** — or derived from the surrounding mode by an **inverting scope**; read by the component tree through token inheritance, an explicit consumer choice wins over the user's OS preference, which is the fallback when no mode is pinned. _Light_ is the default/unset state.
_Avoid_: Theme (a "theme" implies a full named palette swap; a mode is specifically the light/dark axis), colour scheme, skin

**Mode scope**:
An element that re-declares the **semantic tokens** for itself and everything inside it, fixing the **theme mode** for that region. Two kinds: one that *pins* a mode (`data-theme="light"` or `"dark"`, ADR-0053) and an **inverting scope** (`data-theme-invert`, ADR-0054). A pinned scope is an element carrying one of those two values, together with everything inside it — the region whose **semantic tokens** resolve in that pinned **theme mode** (ADR-0053). The document root is the outermost mode scope; scopes nest, and the nearest enclosing one wins, because a scope re-declares the role tokens on itself and inheritance carries them inward — through shadow boundaries and into the **top layer**, since a native popover stays in the DOM tree. Only those two values pin a mode; any other is inert, so a consumer's own `data-theme` system cannot knock components out of the surrounding mode. A scope re-points colour and paints nothing of its own: a container opts into paint with `bg-surface text-on-surface`, and a `color` resolved outside the scope inherits in unchanged. Every scope publishes the mode it resolved as `--c-mode` (`light` or `dark`) — that is how an **inverting scope** reads its surroundings and how `themeMode()` answers.
_Avoid_: Theme scope / theme island ("theme" is the banned word for this axis, and a scope paints no box), forced dark, local theme

**Inverting scope**:
An element carrying `data-theme-invert`, together with everything inside it — a **mode scope** whose **theme mode** is the *opposite* of the one the element sits in (ADR-0054). Relative where `data-theme` is absolute: the same markup reads dark on a light page and light on a dark one, so a region that must always stand apart from its surroundings needs no per-page variant. Inverting scopes nest like pinned ones and the nearest wins, so an inverting scope inside an inverting scope is back in the surrounding mode; a pin on the same element wins over the inversion. At the document root it means the opposite of the OS preference. Presence-only, like `hidden`: `data-theme-invert="false"` still inverts — remove the attribute to stop. Like any mode scope it re-points colour and paints nothing.
_Avoid_: Inverted scope (the scope inverts its surroundings; it is not itself inverted), inverse scope (`inverse-*` is the mode-**invariant** token family), inverted surface (one rung of the **surface ladder**, not a mode), contrast scope, auto dark
