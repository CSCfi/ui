...
instead of anchoring under the field: a heading row with the field's `label` and
a close button, the search input beneath it, then the items filling the rest of
the screen. The panel keeps covering the screen and the rows above the items
stay in view while the on-screen keyboard is open; the close button and Escape
both return focus to the field; the page behind the panel is inert until it
closes, and the list has no half-row cut — the screen edge is the cue. The
...
```css
c-tree-select::part(heading-row) {
