 ...
 */
@Component({
  tag: 'c-text-field-open',
  styleUrl: 'c-text-field-open.scss',
  shadow: false,
  formAssociated: true,
})
export class CTextField {
   ...
   */
  @Prop() renderOutsideShadow = false;
  ...
  @Element() hiddenEl!: HTMLCTextFieldOpenElement;
  ...
  render() {
    const inputElement = this._renderInputElement();
    const passwordToggle = this._renderPasswordToggle();
    const dateToggle = this._renderDateToggle();

    if (this.renderOutsideShadow) {
      return (
        <Host>
          <c-input
          ...
          <div slot="input-slot">{inputElement}</div>
        </Host>
      );
    }
