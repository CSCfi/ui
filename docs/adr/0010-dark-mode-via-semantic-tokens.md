...
- **Brand-preserving vocabulary + neutral additions.** Keep the existing CSC role names — `primary`, `secondary`, `accent`, `success`, `info`, `warning`, `error`, `link` — now mode-aware. **Add** neutral roles that don't exist today: the **surface ladder**, `on-*` foregrounds, `border`, `ring`. Rejected generic functional (shadcn/Radix) and full Material `on-X`/`-container` taxonomies — see alternatives. (ADR-0036 later adds a `divider` neutral role — the translucent hairline ink.)
- **Per-role token set, finalized in the `CButton` pilot.** Each brand/status role carries six tokens: `<role>` (solid fill / brand text), `<role>-hover`, `on-<role>`, `<role>-subtle` (tint fill, palette step 200), `<role>-subtle-hover` (step 100), `on-<role>-subtle`. Explicit interaction/tint tokens (rather than opacity modifiers) were chosen so CSC's lighten-on-hover reads precisely in both modes.
- **Mode-invariant `inverse-*` family.** A small set (`inverse-surface`, `inverse-on`, `inverse-primary`, `inverse-error`) with *identical* light and dark values, emitted once under `:root` (from `tokens/semantic/invariant.json`). It backs the `inverted` variants, which sit on a fixed brand/dark backdrop and must keep their look regardless of theme — so they must not flip. Theme-relative tint shades for inverted are expressed as opacity on these (e.g. `bg-inverse-on/20`).
