<template>
  ...
  <component-example name="simple" rows>
    ...
    <div>
      <c-tags>
        <c-tag v-for="country in filteredCountriesSimple" :key="country">
          {{ country }}
        </c-tag>
      </c-tags>

      <c-pagination v-model="optionsSimple" v-control hide-details simple />
    </div>
...
const filteredCountriesSimple = computed(() =>
  countriesList.slice(
    optionsSimple.value.startFrom,
    (optionsSimple.value?.endTo || 0) + 1,
  ),
);

const optionsSimple = ref<CPaginationOptions>({
  itemCount: countriesList.length,
  itemsPerPage: 25,
  currentPage: 1,
  startFrom: 0,
...
const setPageToTwo = () => {
  optionsSimple.value = { ...optionsSimple.value, currentPage: 2 };
};

const showFiftyItemsPerPage = () => {
  optionsSimple.value = { ...optionsSimple.value, itemsPerPage: 50 };
};
// @example-end
</script>
