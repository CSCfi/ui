...
| 12 | Sub side-nav item background | Sub-item states paint page roles (`surface-raised`, `primary-subtle-hover`, `primary`) inside the `nav-active` pill (`CSideNavigationItem.vue:148`, `CSubNavigationItem.vue:56-89`). | **Translucent `on-nav-active` washes** (ADR-0052) for hover (10 %), the indicator bar and the focus ring; the active fill is the new `nav-sub-active` role — white in light mode (the look the light theme already had, restored after review), an 18 % `on-nav-active` wash in dark. New side-navigation spec + baselines. |
