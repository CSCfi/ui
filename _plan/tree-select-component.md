...
**Outcome (2026-09-11):** all gates green (strict analyzer 0 errors, `vue-tsc`, `lint:a11y`, `lint:tokens`, React codegen, docs example parity + `nuxt build`). Smoke run: **132 checks passed, 0 failed** in light + dark (scratchpad `smoke.mjs`, `demo.html`, `cdp.mjs`). Two field bugs found after the first pass and fixed: the empty field's chevron sat under the label because the value block was `v-if`'d away (the block is now always rendered as the row's filler), and an empty `v-model` floated the label and showed the clear button because Vue's `vModelText` writes `el.value = ''` for a `null` model (the empty string now normalizes to `null` in `valueOf`, as `!!value` does in the siblings). A third fix landed in the docs only: the Vue canons bound arrays and functions with Vue's `.prop` modifier, which forces the literal key and never camelizes, so `:level-labels.prop` wrote a dead `el['level-labels']` and the header read "Choose Level 1". A plain binding lands as a property on a Vue custom element (camelCase keys via the declared-prop accessors, hyphenated keys via Vue's `_isVueCE` branch), so `.prop` was dropped from every Vue canon and from the usage.md guidance, and `scripts/check-example-parity.mjs` now rejects `.prop` in canons. The pre-existing c-data-table canons `selection.vue`/`expansion.vue` carried the same bug (`:get-row-id.prop`, `:expanded-content.prop`; the expansion demo rendered nothing) and were fixed in the same sweep. One real bug found and fixed in both components: a click on a disabled row (`pointer-events: none`) fell through to the `<ul tabindex="-1">`, which took DOM focus from the search input — fixed with `@mousedown.prevent` on the list. Observed, not changed: c-autocomplete's in-panel search input has no explicit `role="combobox"` (implicit textbox), unlike the glossary's description; c-tree-select mirrors it.
